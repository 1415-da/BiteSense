            ...
            <motion.div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <motion.div style={{ flex: '1 1 280px', minWidth: 0 }}>
                ...
                <p style={{ color: 'var(--text-secondary)', marginBottom: '1.25rem', lineHeight: 1.55, fontSize: '0.95rem' }}>
                  These choices are enforced as filters and safety checks when we plug in the dish-level model—so allergens stay excluded and guardrails stay respected.
                </p>
                <AnimatedGlassCard hoverLift={false} style={{ marginBottom: '1.35rem', background: 'rgba(22, 28, 36, 0.5)' }}>
                  ...
                  <p style={{ margin: '0.65rem 0 0', fontSize: '0.88rem', color: 'var(--text-secondary)' }}>
                    <strong style={{ color: 'var(--text-primary)' }}>Guardrails:</strong> max {health.max_sodium_mg} mg sodium · max {health.max_sugar_g} g added sugar / day
                  </p>
                </AnimatedGlassCard>
                <motion.button
                  type="button"
            ...
            <motion.div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '1rem', marginBottom: '1rem' }} variants={staggerContainer} initial="initial" animate="animate">
              <AnimatedGlassCard>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
                  ...
                  <h3 style={{ fontSize: '1.05rem', margin: 0 }}>Allergens to avoid</h3>
                </div>
            ...
            <AnimatedGlassCard style={{ marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  ...
                  <input type="range" min={1200} max={3000} step={50} value={maxSodium} onChange={(e) => setMaxSodium(Number(e.target.value))} style={{ width: '100%', accentColor: 'var(--accent-primary)' }} />
                </div>
                <div>
                  ...
                  <input type="range" min={10} max={80} value={maxSugar} onChange={(e) => setMaxSugar(Number(e.target.value))} style={{ width: '100%', accentColor: 'var(--accent-primary)' }} />
                </div>
              </div>
              <motion.div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.65rem', marginTop: '0.75rem' }} variants={staggerContainer} initial="initial" animate="animate">
                <AnimatedMetricTile>
